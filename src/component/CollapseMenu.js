...
// this is the end of the layers control plugin code

L.mapbox.accessToken = 'pk.eyJ1IjoiZG9zcyIsImEiOiI1NFItUWs4In0.-9qpbOfE3jC68WDUQA1Akg';
var map = L.mapbox.map('map', null)
    .setView([40, -74.50], 9);
	
var grey = L.mapbox.tileLayer('mapbox.light');
...
var seaplane = L.geoJson(myData, {
    filter: function(feature) {
        switch (feature.properties.Subcategory) {
            case 'Seaplane Base': return true;
                break;
        }
    },
    onEachFeature: onEachFeature
...
var railroad = L.geoJson(myData, {
    filter: function(feature) {
        switch (feature.properties.Subcategory) {
            case 'Railroad': return true;
                break;
        }
    },
    onEachFeature: onEachFeature
...
var waterTaxi = L.geoJson(myData, {
    filter: function(feature) {
        switch (feature.properties.Subcategory) {
            case 'Water Taxi': return true;
                break;
        }
    },
    onEachFeature: onEachFeature
...
var groupedOverlays = {
  ...
  "<span class='controlHeading'>Aviation</span>": {
	"airport": airport,
	"heliport": heliport,
	"seaplane": seaplane
  },
  "<span class='controlHeading'>Transportation</span>": {
	"railroad": railroad,
	"water taxi": waterTaxi
  }
};
